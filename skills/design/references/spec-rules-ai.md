...
| `specs/spec-001.md` | The worked example. The only spec kept in this repo |
| `~/gsl-specs/spec-NNN.md` | Every other spec, written by the design skill. `NNN` is three digits, the highest number in that folder plus one. **An empty folder starts at `002`**, so no spec shares the worked example's number. Never reused |

**Specs live outside every repository, so a spec outlives its prototype.** A
build skill works on a throwaway branch and deletes it at cleanup; a spec saved
there would be deleted with it. One folder also lets the same spec be built on
every platform. Gabriel, 24 September 2026.
...
**A spec may be waiting for its goals to be validated:** `## Screen` and
`## Inventions` then say `None.`, the status is `draft`, and a build agent
given it builds nothing and says the screen is not designed yet. Gabriel,
24 September 2026, for the design skill, which designs no screen until the
person has validated the goals and acceptance criteria.
