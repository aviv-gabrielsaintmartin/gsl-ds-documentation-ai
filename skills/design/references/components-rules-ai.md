...
not read. Full inventory: components-index.md._
...
_**Stated here and in full in [icons-rules-ai.md](icons-rules-ai.md)**,
...
| Name | Purpose | Doc |
| --- | --- | --- |
| `Accordion` | Accordions are container that allow users to expand and collapse sections of content, making it easier to manage large amounts of… | accordion |
| `Action Menu` | Action menus display context-specific actions in a dropdown list. | action-menu |
| `Alert` | Alerts are modals that provide users with critical information they need immediately. | alert |
| `Autocomplete` | Autocomplete components suggest possible matches for user input in real time as they type, helping them complete text fields more… | autocomplete |
| `Avatar` | Avatars represent user profiles of agencies, agents, private sellers and seekers. | avatar |
| `Badge` | Attention marker attached to a host component. | badge |
| `Badge Store` | Our replicas of the official App Store and Google Play badges, kept here so they can be maintained. | badge-store |
| `Brand Logo` ⚠︎ *also in Foundations* | 🚫 **Never select** — asset — brand config | — *no doc* |
| `Button` | Buttons are used to trigger an immediate action. | button |
| `Button Bar` | Holds the actions that close a form or a flow, anchored at its foot. | button-bar |
| `Button Card` | 🚫 **Never select** — withheld — never developed, available on no platform | button-card |
| `Button Card Group` | 🚫 **Never select** — withheld — never developed, should leave Figma | — *no doc* |
| `Button Group` | Button groups display multiple related choices in a horizontal row, allowing users to select one or more options. | button-group |
| `Card` | Cards are flexible containers used to visually group content. | card |
| `Carousel` | Carousels are used to display a collection of items that the users can slide through. | carousel |
| `Cell Content` | 🚫 **Never select** — composed-only — a slot inside Cards and lists | cell-content |
| `Checkbox` | Checkboxes are used to select one or more options from a list. | checkbox |
| `Checkbox Group` | Checkbox groups are used to select multiple options from grouped checkboxes. | checkbox-group |
| `Chip` | Chips are used to filter content, make selections, display input information or trigger actions. | chip |
| `Chip Group` | Chip groups are collections of chips that allow users to filter, select, or manage multiple related options simultaneously. | chip-group |
| `Coachmark` | Coach marks are temporary overlay messages that provide contextual information about user interface elements. | coach-mark |
| `Content Placeholder` | 🚫 **Never select** — composed-only — a slot, swapped for local content | — *no doc* |
| `Counter Field` | Counter fields are used to enter or select numeric values. | counter-field |
| `Divider` | Dividers are horizontal lines that separate content. | divider |
| `Dropdown` | Dropdowns are used to select one option from a list. | dropdown |
| `Energy Tag` | Energy tags are used to indicate the energy efficiency of properties. | energy-tag |
| `Feedback Messages` | Feedback messages are non-disruptive, inline notifications that provide users with important information or contextual messages. | feedback-message |
| `Feedback Thumb Buttons` | Asks the user for a binary opinion — thumbs up or thumbs down. | feedback-thumb-buttons |
| `Floating Button Group` | The floating button group is used to display icon-only actions on top of images and maps. | floating-button-group |
| `Image Ratio` ⚠︎ *also in Foundations* | 🚫 **Never select** — withheld — a Figma-internal ratio helper for designers | — *no doc* |
| `Image Slider` | Horizontally sliding image sequence. | image-slider |
| `Link` | Links are navigational elements that are used to direct users to another location or resource. | link |
| `Loading State` | Signals data or content is being fetched. | loading-state |
| `Modal Bottom Sheet` | Modal bottom sheets are containers that appear above the content and block interaction with the rest of the screen. | modal-bottom-sheet |
| `Modal Bottom Sheet Menu` | Modal bottom sheet menus display a list of context-specific actions on mobile screens or on apps. | modal-bottom-sheet-menu |
| `Navigation Bar (App)` | Persistent in-app navigation between top-level destinations. | navigation-bar-app |
| `Pagination` | Pagination divides content into smaller, numbered pages, making it easier for users to navigate through large amounts of content. | pagination |
...
| `Progress Bar` | A progress bar shows a task's progress. | progress-bar |
| `Progress Circle` | A progress circle shows a task's progress. | progress-circle |
| `Radio Button Group` | Radio button groups are used to select one option from a group of mutually exclusive choices. | radio-button-group |
| `Rating` | The rating is used to display the result of user ratings. | rating |
| `Score Tag` | A Tag specialised for seller lead scoring. | score-tag |
| `Segmented Control` | Segmented controls are used to select one option from a group of mutually exclusive choices. | segmented-control |
| `Select Card Group` | Select cards are used for single- or multi-selection inside forms. | select-card-group |
| `Slider` | A range slider can be used to select a single value or a range between minimum and maximum values. | slider |
| `Snackbar` | Snackbars are used to provide quick feedback after an action is taken. | snackbar |
| `State Message` | Inline feedback inside a form field. | state-message |
| `Tab Bar` | 🚫 **Never select** — withheld — mid-refactor, use Tabs | — *no doc* |
| `Tabs` | Tabs are used to organize related content into different views and allow users to seamlessly switch between them. | tabs |
| `Tag` | Tags are used to label, categorize and highlight items to help users quickly identify content. | tag |
| `Text Area` | Text areas are used to enter and edit multi-line text content. | text-area |
| `Text Button` | A distinct component from Button, for when full button weight is too heavy. | text-button |
| `Text Field` | Text fields are used to enter and edit single-line text content. | text-field |
| `Toggle` | Toggles are used to switch between on and off states. | toggle |
| `Toggle Group` | Toggle groups are used to organize related options, allowing users to switch between multiple settings, with each toggle independently… | toggle-group |
| `Tooltip` | Brief overlay clarifying one UI element. | tooltip |
...
| Name | Purpose | Doc |
| --- | --- | --- |
| `Burger menu` | Mobile menu opened from the navigation bar burger icon. | burger-menu |
| `Burger menu (profil)` | 🚫 **Never select** — withheld — adapted to consumer content, use Burger menu | — *no doc* |
| `Bar graph` | Compare quantities across categories. | charts/bar-chart |
| `Donut chart` | Show a distribution across parts of a whole. | charts/donut-chart |
| `Line chart` | Show a trend over a continuous axis. | charts/line-chart |
| `KPI` | Key Performance Indicators (KPIs) are measurable values that demonstrate how effectively a key objective is achieved. | kpi |
| `Breadcrumb` | Breadcrumbs are navigation elements that consist of a list of links arranged in a hierarchical order. | breadcrumb |
| `Date Field` | Date input — distinct from the Date Picker calendar view. | date-field |
| `Date Picker` | Date pickers are used to select a date using text input or a calendar view. | date-picker |
| `Feedback Bar` | Asks the user to rate something — a notation on a scale. | feedback-bar |
| `Filter bar` | Filter bars are used to narrow down search results or displayed content based on selected criteria. | filter-bar |
...
| `Info State` | Info states are placeholders used to inform users about success, error and empty states. | info-state |
| `Media Upload` | Media upload components allow users to upload, view, and manage media files such as images, videos and documents. | media-upload |
| `Mega menus` | Top-level navigation on the main B2C and B2B websites. The Figma was built by the design system team; the component is not stored here yet and the transfer is ongoing. | mega-menus |
| `Menus` | 🚫 **Never select** — withheld, provisional — nothing requires it today, use Navigation bar | menus |
| `Navigation bar` | Navigation bars provide quick access to key pages within the site, helping users to navigate efficiently. | navigation-bar |
| `Top Bar` | Top bars display navigation elements, titles and actions such as buttons or icons at the top of the screen. | top-bar |
| `Wizard` | Wizards guide users through step-by-step processes to achieve their goal. | wizard |

#### Experiences library

| Name | Purpose | Doc |
| --- | --- | --- |
| `Estimation card` | Presents a completed price estimate — range, confidence, selling or renting. Carries no controls to adjust it. | estimation-card |
| `Floor selection` | Picking an apartment floor, including ground floor. | floor-selection |
| `Listing Card` | Listing cards are actionable cards that summarize the details of a property listed on any AVIV Group website. | listing-card |
| `Listing summary` | Higher-flexibility alternative to Listing card. | listing-summary |
| `Map template` | The map experience container. | map-template |
...
| `Phone Number Field` | The phone number field is used to input and format phone numbers. | phone-number-field |
| `Table` | Tables are used to organize and display all information from a data set. | tables |
...
3. If nothing still fits, **compose from existing Components** using the token
   rulesets — [colour](color-rules-ai.md),
   [typography](typography-rules-ai.md),
   [spacing](spacing-rules-ai.md). Never hand-style something
