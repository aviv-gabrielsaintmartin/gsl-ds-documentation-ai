...
**This folder holds instructions written here and used somewhere else.** Two
so far:

- `gsl-color-rules/SKILL.md` — for Figma's design agent. Everything below this list is about it.
- `design/` — the design skill: an idea to a spec, built by `/prototype` in the iOS repo. Its `references/` folder is generated by `scripts/build-design-references.py`, never edited. The workflow is in `project/how-a-prototype-is-made.md`.
