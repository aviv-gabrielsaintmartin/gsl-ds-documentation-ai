...
### Three options, and when each is the right answer

| Option | Use it when |
| --- | --- |
| `--place "Name=Target"` | The draft script printed a guessed placement and you checked it and it was wrong. Repeatable. **Write the correction here rather than moving the section by hand afterwards** — by hand it is lost on the next run, and nobody knows a human ever decided it |
...
A `--place` correction is a decision. Record it in the run's own command so the
next session can see what was decided and why the section is not where the
draft script put it.
