...
1. Determine the tier: if the user named it explicitly, use that. Otherwise infer it from the open file's name — `cd ~/figma-cli && node src/index.js eval 'JSON.stringify({name: figma.root.name})'` — and match that against `figma-libraries-registry.json`. **Match on the file *name*, never on `fileKey` — and this is the better rule, not a workaround.** A tier is a design-system concept that will be used on web, iOS and Android too; a file key is Figma-specific and means nothing on any of them (Gabriel, 14 Sep, the same reasoning that keeps keys off the icon pages). It also happens to be the only thing that works: `figma.fileKey` is `undefined` through FigCli and no channel on this machine exposes it. If the name matches no tier, ask rather than guess.
