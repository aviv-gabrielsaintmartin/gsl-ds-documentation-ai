...
**`scripts/check-all.py` runs all seven checks and prints one verdict.** Run it
...
**A file that examines another file is named after it, in full** — `components-rules-ai-eval.md` tests `components-rules-ai.md`, `compliance-scorecard-audit.md` explains `compliance-scorecard.md`. **The boundary: a file about a topic keeps the topic's name**, which is why `color-usage-audit.md` and `compliance-scorecard.md` stay as they are. Gabriel, 23 September 2026. The rule and its worked examples are in `README.md`.
