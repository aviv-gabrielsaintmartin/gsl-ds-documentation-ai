...
| `scripts/`        | `check-links.py` — run it after renaming or deleting anything. It reports links whose target is gone, filenames written in backticks that no longer name a file, and rulesets that point an agent at evidence it may not read. `check-rules-docs.py` — run it after editing a component doc or the ruleset; it reports where the two send an agent to different places                                                                                                       |
