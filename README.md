...
**`scripts/check-all.py` runs all seven of those and prints one verdict.**
...
| Find the iOS name of a component, a token or an icon | [components/components-ios-map.md](components/components-ios-map.md) · [tokens/tokens-ios-map.md](tokens/tokens-ios-map.md) · [icons/icons-ios-map.md](icons/icons-ios-map.md) |
...
| `components/` | **76 component docs** across 69 folders — `charts/` holds eight pages, three of them components. 55 folders carry a self-contained `images/`. **Ten folders also hold a `-figma.md`** tool specification. Plus the ruleset, audit and eval                                                                                                                                |
...
| `compliance/` | How generated output is judged — eight yes-or-no questions, grouped under the step of designing each one guards, and the report every run must produce. Read by a **checking** agent, not a generating one. `compliance/briefs/` holds one folder per brief — the brief itself, written once, and one folder per run of it holding the screenshots and the report | [compliance/compliance-scorecard.md](compliance/compliance-scorecard.md) |
...
| `scripts/`        | **`check-all.py` — run it after changing anything under `components/`.** It runs the seven standing checks and prints one verdict, and changes nothing unless `--write` is passed. The seven: `check-links.py` (links, backticked filenames, and rulesets pointing an agent at evidence it may not read), `check-rules-docs.py` (where a doc and the ruleset send an agent to different places), `check-rules-reach.py` (a selectable component no rule names, so no agent can ever pick it), `check-tool-neutral.py` (a doc explaining itself with a tool), `check-readiness-figma.py` (a readiness table's Figma cell that disagrees with the Figma registries), and the two generated pages, checked for still being current. **A clean run proves the docs are well-formed. It never proves a sentence is true** — every check reads this repo against itself |
