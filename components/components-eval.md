...
| 17 | "Confirm before deleting a listing, blocking, **on web**" | `Pop-up` for a short confirmation, `Modal bottom sheet` for richer content — **never** `Alert` | **Platform limits overrides Which component.** Alert is not yet available on web. Either answer passes; `Alert` fails |
...
| 27–30 | Usable. Fix the missed rows before extending **Which component** |
| ≤26 | The ruleset is not yet the single entry point. Do not point agents at it |

**Intents 28–31 have not been run.** They were added on 2026-09-08 with **Highest tier first**'s
