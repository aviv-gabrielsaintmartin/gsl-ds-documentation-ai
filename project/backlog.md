...
| What | What goes wrong today | How often | How long |
| --- | --- | --- | --- |
| Write a human walkthrough of how a run gets reported | **The output doesn't change — this one is for you, not for an agent.** Gabriel asked on 11 Sep how he would explain the reporting pipeline to someone else, and the repo cannot answer: the reasoning exists only in a `project/decisions.md` entry, which records *why* rather than *how it works*, and in a conversation that will be gone. Goes in `project/how-a-run-is-reported.md` — that folder is the one place in the repo written human-first. **Must explain and never specify:** the flow from brief to report, the folder tree after several runs, who writes and reads each file, and two walkthroughs — you three weeks later asking "is it getting better", and the checking agent on its next run. It **links** to the scorecard for the template and the required facts and never repeats them, or the two drift and there are two answers. **Written for a non-technical reader**, with diagrams. ASCII in code fences, matching `status.md` and `CLAUDE.md` — it renders everywhere, including a plain editor, where Mermaid would show as source. **Not published anywhere** — Gabriel's call, 11 Sep: documentation on his side only | Whenever he explains this to anyone | An hour |
