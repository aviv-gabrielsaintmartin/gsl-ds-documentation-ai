...
| --- | --- | --- | --- |
| Fix two rules files that still name renamed files, and teach the link check to see bare filenames | **Stops or asks, and it is a defect we just created.** `.claude/rules/component-docs.md:22` tells a session the component index is `components.md`; it is `components-index.md`. `.claude/rules/token-docs.md:16` says the same about `tokens.md`. Both are live instructions naming files that no longer exist. `scripts/check-links.py` missed them because it only reads `[text](target)` links, not names in backticks — a rename check that only sees links is half a check | Every session that opens either rules file | Half an hour |
...
| Fix the nine places where a ruleset sends an agent to evidence it may not read | **Stops or asks.** `scripts/check-links.py` now names all nine exactly: two in the component ruleset, seven across colour, radius, shadow, spacing and typography. Most are header notes saying where the evidence lives; `components-rules-ai.md:292` is the bad one — a rule that stops mid-answer and says see the audit | Every time an agent follows a ruleset to the end | Half a day |
