...
| What | What goes wrong today | How often | How long |
| --- | --- | --- | --- |
| Point the two places that list this repo's human-first pages at the new walkthrough | **The output doesn't change — only our own files.** `project/how-a-run-is-reported.md` exists and nothing links to it. `README.md`'s *Where to look* table and the file table in `.claude/rules/project.md` both enumerate what lives under `project/`, and both now list one page fewer than there are. A page nobody can find from the map is a page that gets rewritten from scratch by the next session that needs it | Every time someone looks for it | Ten minutes |
