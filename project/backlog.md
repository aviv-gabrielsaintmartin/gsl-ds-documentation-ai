...
| Write a link check — every link in a ruleset resolves, and points at a file agents are allowed to read | **Stops or asks**, and it is the only *preventive* item here. Three of the tasks on this list are dead links found by hand, one at a time. About twenty lines of Python, run whenever we want. It catches the whole class instead of one case | Every time anything is deleted or renamed | An hour |
...
| Record the Figma text-field names for each component | **Ships wrong, breaks no rule.** An agent can place a component but has no way to put words into it, so it ships with the default placeholder text still in place — which breaks nothing. Every string in the test came from reading Figma live, not from this repo | **A normal foreground session, plus Figma Desktop open.** Background jobs load no Figma tools at all, whatever your settings say. Check the bridge responds before starting, not after | A day |
...
| 10 Sep 2026 | **The Figma tools are missing from background sessions, not from your setup.** The `figma` plugin is enabled in `~/.claude/settings.json` and your four personal skills are correctly installed in `~/.claude/skills/` — but a background job loads none of them. So the text-field-names task is not waiting on you to switch anything on; it needs to run in a normal foreground session. Check `figma_get_status` exists before starting it, not after |
...
| 10 Sep 2026 | **The graphify graph was tested and it is confidently stale — do not use it as it stands.** Asked what the `-rules-ai` suffix means, it cited `project/plan.md`, `project/briefs/00-doc-layer.md` and `01-compliance.md` — archived on 9 Sep — and `design-language/typography_04.pdf`, from a folder that was deleted. It also surfaced "the block ritual — brief, build, check, log", the retired four-step process, as if current. A session trusting it would be handed the old way of working. Same failure as the one already recorded on 9 Sep, from a different direction |
...
| 10 Sep 2026 | **Graphify flags `tokens/tokens.md` as "potentially sensitive" and skips it.** The word *tokens* trips its secrets heuristic. So even a successful build silently omits the token index — the file that says what every other token file is for. Worth knowing before reading any graph as complete |
