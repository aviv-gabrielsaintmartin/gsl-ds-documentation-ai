...
too wide, which is still an open question about the component itself. The
routing-entry task is still worth holding against the same fallback rule.
...
| 10 Sep 2026 | **The scoped rebuild could not be run from a background session.** `graphify extract --backend claude` needs `ANTHROPIC_API_KEY`, which isn't set. The other route is `/graphify`, where Claude itself does the reading. Nothing was lost: the old graph was backed up and restored, and it is still the stale one from 9 Sep. Do not trust it until it is rebuilt. **The claim that `/graphify` only loads in a foreground session was wrong** — it loaded and ran in a background session on 10 Sep. It had been assumed, not tested |
...
| 10 Sep 2026 | **What a graph refresh actually costs, measured rather than assumed.** An update over 15 changed files cost **176,040 tokens** — about 11,700 a file, roughly double what the files' own word count predicts, because it includes the extraction agent's reasoning and tool calls. The full build before it was 814,930. An update re-reads each changed file **whole**; there is no diffing. So the price is set by which files changed, and the two that change on *every* task were the two biggest: `project/backlog.md` at ~9,000 input tokens and `status.md`. Both are now in `.graphifyignore` — every session opens them directly, so nothing ever looked them up through the graph — and the refresh step in `/task-check` now runs only when a task changed something else. Gabriel caught this by asking why an "update" was costing six figures; it had been added to the loop without anyone measuring it. **Even with both guards a task that edits one ruleset costs roughly 10,000–30,000 tokens.** If that stays annoying, take the step out of the loop and refresh the graph by hand before querying it |
| 10 Sep 2026 | **`graphify update .` only does a structural pass.** It pruned the old filenames and picked up the new ones as heading-level nodes, but the richer concept extraction needs `/graphify --update` through the skill. The step in `/task-check` keeps the graph honest about what exists, not deeply informed about what changed. **Superseded the same day:** the check step now calls the skill, because almost every task here rewrites a rule without moving a file — exactly what the structural pass cannot see. It costs tokens where the shell command was free; that is the accepted trade |
