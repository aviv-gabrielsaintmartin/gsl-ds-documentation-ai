...
| Give the two rules files names that say what they are — `.claude/rules/tokens.md` → `token-docs.md`, `components.md` → `component-docs.md`, each with a first line saying it is an instruction, not content | **Stops or asks.** Today `.claude/rules/tokens.md` and `tokens/tokens.md` share a name, and so do the two `components.md`. The instruction file's own heading reads "Token docs", which sounds like content. A session arriving by `grep` gets no frontmatter to tell it apart. Nothing links to a rules file by name, so this costs nothing. `figma-registries.md` already sets the precedent that these are named for their subject | Every time anyone opens the wrong one — it happened to Gabriel in the session that found it | Half an hour |
| Add `-index` to the filename grammar and rename the two routing pages — `tokens/tokens.md` → `tokens-index.md`, `components/components.md` → `components-index.md` | **Stops or asks.** `CLAUDE.md` says the suffix tells you what a file is. These two are the only files in the repo that break that rule, so their names say nothing — `tokens.md` could be a list of tokens, a usage record, or a routing page. It is a routing page and contains no tokens at all. **Do the link check first**, then this: ~12 links move, plus the grammar table in `CLAUDE.md` and `README.md` | Whenever someone has to guess what a file holds | An hour |
