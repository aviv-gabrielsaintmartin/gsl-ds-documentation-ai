...
| **C2** | **Tier ceiling** | Was a higher-tier component reinvented from lower-tier parts? | Rule 0's ten rows, using the ruleset's own *count the parts* test: a local frame reproducing **two or more** of a higher-tier component's moving parts is a violation |
...
| **C5** | **Declaration** | When something new was invented, was it declared? | Rule 5 requires stating what was built, which problem it belongs under, and which components were ruled out and why |

**C4 is the check the token audits paid for.** 83 of 218 colour tokens have no
consumer, `Spacing/56` has no documented purpose, and no component binds a
Display type style. Without C4, output can be 100% "token-bound" and still be
...
| **The no-false-precision test** | Nothing is scored that rests on unverified rules — which is why C6 ships switched off |
...
| 3 | Is C2 reliably machine-detectable, or a heuristic? | **Machine-detectable.** New or hand-built components must be flagged | Correct, but not from the registry: `Listing Card`'s `exposedSubComponents` are private internal slots (`.listing_price_tag`), not the public components someone would reinvent it from — and only 5 of 11 Experiences record them at all. So C2 needs a **parts column on Rule 0's ten rows**, and uses C5 as the tiebreaker for false positives |
...
The first correction also resolved two of the six open questions: Rule 0's parts
...
| **False precision.** A tidy 87% implying more rigour than the evidence supports | C6 ships inactive. Every check states what it cannot see |
| **It measures what is easy rather than what matters.** C3 is trivial to check; C2 is where real compliance failures live | C2 is defined even though it needs a human confirm, rather than dropped for being awkward |
